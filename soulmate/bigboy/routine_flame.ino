namespace Flame {
  ...
  void flame_paint(CRGBPalette256 palette, bool shift_hues) {
    ...
    for(int i=0; i<N_CELLS; i++){
      ...
      Soulmate::led_arr[i] = ColorFromPalette(palette, heatindex); // normal palette access
      ...
      if((heatindex==0) && (Util::rand_range(0, 10000)==0)){
        Soulmate::led_arr[i] = ColorFromPalette(palette, Util::rand_range(0, 40));
      }

      if(shift_hues) {
        // Shift hue. This is really inefficient, but I don't know how to
        // shift the hue within the palette.
        CHSV hsv = rgb2hsv_approximate( Soulmate::led_arr[i] );
        hsv.hue += flame_hue_shift;
        Soulmate::led_arr[i] = hsv;
      }
    }
